...
from globe.GlobeScore import GlobeScore
import numpy as np
import itertools
import os
...
def score_candidate(score,network,data, child, curr_parents, op):
        # This is executed during the forward phase
        pa_pa = np.nonzero(network[:, op])[0].tolist()
        ch_pa = curr_parents
        s_12 = score.compute_gain(data, child, ch_pa.copy(), op)
        s_21 = score.compute_gain(data, op, pa_pa.copy(), child)
        phi = s_12 - s_21
        return op,child,phi
...
class GreedyDagSearch:
    def __init__(self, **kwargs):
        ...
        self.gu = GraphUtil()
        self.workers = 1#math.floor((os.cpu_count() - 3)/2)
        #print(f"Available CPUs: {os.cpu_count()}")
        #print(f"Using CPUs: {self.workers}")
    ...
    def forward(self,headers):
        ...
        while pq.hasMore():
            ...
            # score candidates in parallel
            args = [(self.score,self.network,data, child, curr_parents, op) for op in cand_parents]
            futures = {
                self.executor.submit(score_candidate, *arg): arg for arg in args
            }
            concurrent.futures.wait(futures)
            for future in futures:
                result = future.result()
    ...
    #def score_candidate(self, data, child, curr_parents, op):
        ...
        #ch_pa = curr_parents
        #s_12 = self.score.compute_gain(data, child, ch_pa.copy(), op)
        #s_21 = self.score.compute_gain(data, op, pa_pa.copy(), child)
        #phi = s_12 - s_21
        #print(phi)
